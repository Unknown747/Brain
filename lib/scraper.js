...
/**
 * Fetch HTML dengan dukungan conditional GET (ETag / Last-Modified).
 *
 * @param {string} url
 * @param {{etag?: string, lastModified?: string}} [meta] metadata cache URL
 * @returns {Promise<{ status: 200|304, html?: string, etag?: string, lastModified?: string }>}
 *   - status 304 → server bilang tidak berubah, html tidak ada.
 *   - status 200 → ada html baru + ETag/Last-Modified terbaru bila server kirim.
 */
async function fetchHtml(url, meta = null, timeoutMs = 20_000) {
    const headers = {
    ...
    // Pakai parse:"raw" agar bisa baca header & status. Status 304 di-allow.
    ...
    if (res._allowed && res.status === 304) {
        return { status: 304 };
    }
...
async function scrapeUrls(urls, cache) {
    ...
    const urlsMeta = (cache && cache.urls) || {};
    ...
    const tasks = urls.map((u) => async () => {
        try {
            const t0  = Date.now();
            const fr  = await fetchHtml(u, urlsMeta[u]);
            if (fr.status === 304) {
                return { url: u, notModified: true, ms: Date.now() - t0 };
            }
            const html = preStripNoise(fr.html);
            ...
            return {
                url: u, priority, phrases, words, years,
                etag: fr.etag, lastModified: fr.lastModified,
                ms: Date.now() - t0,
            };
        } catch (e) {
    ...
    for (const r of results) {
        ...
        if (r.notModified) {
            notModifiedCount++;
            logger.info(`${r.url} → 304 Not Modified, dilewati (${r.ms}ms)`);
            continue;
        }
        // Simpan ETag/Last-Modified terbaru ke cache untuk sesi berikutnya.
        if (cache && (r.etag || r.lastModified)) {
    ...
    if (cache && (skipWord || skipPhrase)) {
        logger.info(`Kata/frasa yang sudah pernah dibuat di cache (di-skip): ${skipWord} kata + ${skipPhrase} frasa`);
    }
    if (notModifiedCount > 0) {
        logger.info(`${notModifiedCount}/${urls.length} URL tidak berubah (HTTP 304) — fetch & parsing dilewati total.`);
    }
