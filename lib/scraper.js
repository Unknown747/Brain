 ...
 *
 * Mengekstrak TIGA jenis kandidat (urutan output = urutan prioritas audit):
...
// Dipanggil setelah stripHtml untuk membuang noise yang sering muncul di wiki.
function cleanWikiText(text) {
    return text
        // Penanda referensi: [1], [12], [a], [edit], [citation needed], [note 1]
        ...
        // Kategori/tools wiki yang lolos
        ...
        // Sitasi ISBN/DOI/arXiv/PMID
        .replace(/\b(?:ISBN|ISSN|DOI|doi|arXiv|PMID|PMC|OCLC|LCCN)\s*:?\s*[\w./-]+/gi, " ")
        // Navbox links seperti "v t e" atau "v · t · e"
        .replace(/\bv\s*[·•]?\s*t\s*[·•]?\s*e\b/gi, " ")
        // Hatnote umum: "Not to be confused with ...", "See also: ..."
        .replace(/\b(?:Not to be confused with|For other uses,? see|This article is about|Main article|See also)\b[^.\n]*[.\n]/gi, " ")
        // Pengucapan IPA dalam kurung: "(/ˈkoʊbiː ˈbraɪənt/)"
        .replace(/\([^()]*[ˈˌːɪʊəɛɔʌθð][^()]*\)/g, " ")
        // Tahun & tanggal di antara kurung: "(born 1978)", "(1978–2020)", "(c. 500 BC)"
...
function isGoodToken(t) {
    ...
    // Cuma angka & hyphen (mis. tanggal, ID).
...
const NGRAM_SIZES       = [3, 4, 5];   // ukuran sliding window untuk n-gram
const NGRAM_PER_SIZE    = 3;           // window per ukuran per kalimat (awal/tengah/akhir)
...
/**
 * Ekstrak runtutan kata berhuruf kapital — ini sering jadi sumber brainwallet
 * (nama orang, judul karya, lokasi). Contoh: "Kobe Bryant", "Lake Tahoe",
 * "Los Angeles Lakers".
 */
function extractProperNouns(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        // Buang false positive yang sering muncul di awal kalimat.
...
/**
 * Pecah proper-noun multi-kata menjadi sub-frasa & komponen tunggal.
 * Contoh: "Kobe Bean Bryant" → ["Kobe Bean Bryant", "Kobe Bean", "Bean Bryant", "Kobe", "Bryant"]
 * Banyak brainwallet hanya pakai nama depan ATAU nama belakang saja.
 */
function explodeProperNouns(names) {
    ...
    for (const name of names) {
        ...
        // Komponen tunggal yang panjang & bukan kata umum.
...
/**
 * Ekstrak frasa-prioritas LANGSUNG dari HTML mentah (sebelum stripHtml),
 * supaya kita tahu mana yang berasal dari title/heading/blockquote.
 */
function extractPriorityHtml(html) {
    ...
    // <i>, <em>, <b>, <strong> sering memuat judul karya / istilah penting.
    grab(/<(i|em|b|strong|cite|dfn)\b[^>]*>([\s\S]*?)<\/\1>/gi, 2);
    return out;
}

/**
 * Ekstrak teks di dalam tanda kutip "..." atau '...' dari teks yang sudah
 * dinormalisasi. Banyak brainwallet berasal langsung dari kutipan terkenal.
 */
...
/**
 * Saring sumber prioritas → pecah jadi frasa yang panjangnya pas, buang
 * yang terlalu panjang/pendek, hilangkan duplikat (case-insensitive).
 */
function refinePriority(rawPhrases) {
    const seen = new Set();
    const out  = [];
    for (const raw of rawPhrases) {
        // Kalau terlalu panjang, pecah per kalimat dulu.
...
function extractFromText(text) {
    ...
    for (const sentence of sentences) {
        ...
        if (!isNav) {
            // (a) Kalimat utuh dengan jumlah kata pas.
            if (raw.length >= PHRASE_MIN_WORDS && raw.length <= PHRASE_MAX_WORDS) {
                pushPhrase(raw.join(" "));
            }
            // (b) N-gram ukuran 3, 4, 5 — sample window awal/tengah/akhir.
 ...
 * @returns {string[]} priority-baru-dulu, lalu frasa-baru, lalu kata-baru.
 */
async function scrapeUrls(urls, cache) {
    ...
    for (const u of urls) {
        try {
            ...
            // (1) Priority dari tag-tag bernilai tinggi + kutipan dalam teks +
            //     proper-noun bigram (nama orang, judul, lokasi).
            ...
            logger.info(
                `${u} → ${prAdded} prioritas, ${pAdded} frasa, ${wAdded} kata baru ` +
                `(skip ${wSkip + pSkip + prSkip} dari cache)`
            );
        } catch (e) {
    ...
    // Urutan audit: priority → frasa → kata.
    return [...allPriority, ...allPhrases, ...allWords];
}
