...
function extractFromText(text) {
    ...
    const pushPhrase = (raw) => {
        ...
        if (NAV_BLACKLIST.test(lower)) return;
        // Buang frasa yang mayoritas (>70%) stop-words atau tidak punya kata isi.
        const toks    = lower.split(" ");
        const content = toks.filter((t) => !STOP_WORDS.has(t));
        if (content.length < 2) return;
        if (content.length / toks.length < 0.3) return;
    ...
    for (const sentence of sentences) {
        ...
        // Skip kalimat yang dimulai dengan pola navigasi (lebih cepat).
        if (NAV_BLACKLIST.test(raw.slice(0, 4).join(" ").toLowerCase())) {
            // Tetap proses kata tunggal-nya saja.
            for (const w of raw) {
        ...
        // (a) Kalimat utuh kalau panjangnya 4–10 kata.
        if (raw.length >= PHRASE_MIN_WORDS && raw.length <= PHRASE_MAX_WORDS) {
            pushPhrase(raw.join(" "));
        }

        // (b) N-gram 4 & 5 — sample window saja, bukan semua sliding.
        if (raw.length >= 6) {
            pushNgrams(raw, 4, pushPhrase);
            pushNgrams(raw, 5, pushPhrase);
        }

        // (c) Token tunggal (DENGAN filter stop-words & sampah).
        for (const w of raw) {
            if (!isGoodToken(w)) continue;
            const lower = w.toLowerCase();
            if (STOP_WORDS.has(lower)) continue;
 ...
 * Mengembalikan array string yang menggabungkan FRASA dulu (high-value)
 * lalu KATA tunggal. Cocok langsung dipakai oleh `generateVariants`.
 */
    ...
    for (const u of urls) {
        try {
            ...
            for (const p of phrases) {
                ...
                if (seenPhrase.has(k)) continue;
                seenPhrase.add(k);
                allPhrases.push(p);
                pAdded++;
            }
            logger.info(`${u} → ${wAdded} kata, ${pAdded} frasa`);
        } catch (e) {
