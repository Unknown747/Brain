...
function cleanWikiText(text) {
    return text
        ...
        .replace(/\[(?:\d{1,3}|[a-z]|edit|citation needed|note \d+|verification needed|when\?|who\?|clarification needed)\]/gi, " ")
        ...
        .replace(/\b(?:Retrieved from|Categories?:|Hidden categor(?:y|ies):|Namespaces?:|Views?:|Personal tools|Variants|Languages?|Page tools|Toggle the table of contents)\b[^\n]*/gi, " ");
}
...
async function scrapeUrls(urls, cache) {
    ...
    for (const u of urls) {
        try {
            ...
            const propers     = extractProperNouns(text);
