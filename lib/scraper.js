...
// ───────── Ekstraksi tahun ─────────
const YEAR_RE = /\b(1[89]\d{2}|20[0-2]\d|2030)\b/g;
...
function fetchHtml(url, timeoutMs = 20_000) {
...
async function scrapeUrls(urls, cache) {
    ...
    for (const u of urls) {
        try {
            ...
            // Ekstrak tahun SEBELUM cleanWikiText sudah membuangnya — tapi
            // YEAR_RE juga akan menangkap tahun di teks bersih (tahun di prosa).
            for (const y of extractYears(text)) {
