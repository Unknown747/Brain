...
async function balTRX(addresses, limiter, onBatch) {
    ...
    await Promise.all(addresses.map(async (a) => {
        ...
        try {
            ...
            rpcStats.recordOk("TRX", "https://api.trongrid.io");
        } catch {
            out.set(a, 0n);
            rpcStats.recordFail("TRX", "https://api.trongrid.io");
        }
        done++;
...
const COINS = {
    ...
    trx:  { name: "Tron",     derive: deriveTRX,  balance: (a, l, cb) => balTRX(a, l, cb),                                   rate: 15 },
