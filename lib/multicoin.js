...
const { createRateLimiter, chunkArray } = require("./util");
...
async function balBTC(addresses, limiter) {
    const out = new Map();
    for (const batch of chunkArray(addresses, 50)) {
        if (limiter) await limiter();
        try {
            const data = await getJSON(`https://blockchain.info/balance?active=${batch.join("|")}&cors=true`);
...
async function balBlockchair(chain, addresses, limiter) {
    const out = new Map();
    for (const batch of chunkArray(addresses, 100)) {
        if (limiter) await limiter();
        try {
            const data = await getJSON(`https://api.blockchair.com/${chain}/dashboards/addresses/${batch.join(",")}`);
...
async function balTRX(addresses, limiter) {
    const out = new Map();
    await Promise.all(addresses.map(async (a) => {
        if (limiter) await limiter();
        try {
            const d = await getJSON(`https://api.trongrid.io/v1/accounts/${a}`);
...
async function balSOL(addresses, limiter) {
    const out = new Map();
    for (const batch of chunkArray(addresses, 100)) {
        if (limiter) await limiter();
        try {
            const body = { jsonrpc: "2.0", id: 1, method: "getMultipleAccounts", params: [batch, { commitment: "confirmed" }] };
...
const COINS = {
    ...
    ltc:  { name: "Litecoin", derive: deriveLTC,  balance: (a, l) => balBlockchair("litecoin", a, l), rate: 1 },
    doge: { name: "Dogecoin", derive: deriveDOGE, balance: (a, l) => balBlockchair("dogecoin", a, l), rate: 1 },
    trx:  { name: "Tron",     derive: deriveTRX,  balance: balTRX,                                rate: 5 },
    sol:  { name: "Solana",   derive: deriveSOL,  balance: balSOL,                                rate: 8 },
};

// rate-limiter per koin (independen dari EVM)
const limiters = {};
