...
function httpRetrySummary(entries) {
    if (!entries || entries.length === 0) return;
    const rows = [...entries].sort((a, b) => b.retries - a.retries).slice(0, 10);
    const line = `${C.gray}${"─".repeat(72)}${C.reset}`;
    process.stdout.write(`\n${C.bold}${C.white}  Ringkasan retry HTTP${C.reset} ${C.gray}(top 10 host bermasalah)${C.reset}\n`);
    process.stdout.write(line + "\n");
    process.stdout.write(
        `  ${C.gray}${"Host".padEnd(40)}│ ${"Total".padStart(6)} │ ${"429".padStart(5)} │ ${" 5xx".padStart(5)} │ ${" Net".padStart(5)}${C.reset}\n`
    );
    process.stdout.write(line + "\n");
    for (const r of rows) {
        const host = r.host.length > 40 ? r.host.slice(0, 37) + "..." : r.host;
        process.stdout.write(
            `  ${C.white}${host.padEnd(40)}${C.reset}` +
            `${C.gray}│${C.reset} ${C.yellow}${String(r.retries).padStart(6)}${C.reset} ` +
            `${C.gray}│${C.reset} ${(r.by429 ? C.yellow : C.gray)}${String(r.by429).padStart(5)}${C.reset} ` +
            `${C.gray}│${C.reset} ${(r.by5xx ? C.red    : C.gray)}${String(r.by5xx).padStart(5)}${C.reset} ` +
            `${C.gray}│${C.reset} ${(r.byNet ? C.red    : C.gray)}${String(r.byNet).padStart(5)}${C.reset}\n`
        );
    }
    process.stdout.write(line + "\n");
}
