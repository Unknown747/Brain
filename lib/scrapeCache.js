...
function load() {
    try {
        ...
        return {
            ...
            // Metadata HTTP per URL untuk conditional GET (ETag / Last-Modified).
            // Bentuk: { [url]: { etag?: string, lastModified?: string, ts: number } }
            urls:    data.urls || {},
        };
    } catch {
        return { words: new Set(), phrases: new Set(), urls: {} };
    }
}
