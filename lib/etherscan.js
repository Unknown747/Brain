...
const { withRetry, httpRequest } = require("./util");
...
function postJsonRpc(url, payload, timeoutMs) {
    // retries:0 — etherscan sudah merotasi RPC dan punya cooldown sendiri,
    // jadi retry internal hanya akan menunda rotasi ke endpoint lain.
    return httpRequest(url, { method: "POST", body: payload, timeoutMs, retries: 0 });
}
 ...
 */
async function callBatchGeneric(url, requests, timeoutMs, label) {
    const payload = requests.map((r, i) => ({
        jsonrpc: "2.0", id: i, method: r.method, params: r.params,
    }));
    if (label) rpcStats.recordStart(label, url);
    let recorded = false;
    const finishOk   = () => { if (label && !recorded) { rpcStats.recordOk(label, url);   recorded = true; } };
    const finishFail = () => { if (label && !recorded) { rpcStats.recordFail(label, url); recorded = true; } };
    try {
        ...
        if (isPayloadTooLarge(e) && requests.length > 10) {
            // Hitung sebagai 1 fail awal, lalu coba ulang via split (sub-call tanpa label).
            finishFail();
...
async function executeOnChain(chainId, requests, label) {
        ...
        if (picks.length >= 2) {
            try {
                const winner = await Promise.any(picks.map(async (p) => {
                    const r = await callBatchGeneric(p.url, requests, timeout, label);
                    return { r, ...p };
                }));
                ...
                for (const p of picks) markCooldown(p.url);
                lastErr = e;
            }
        }
    }
    if (!data) {
        ...
        for (let off = 0; off < urls.length; off++) {
            ...
            try {
                data = await withRetry(() => callBatchGeneric(url, requests, timeout, label), 1, 300);
    ...
    if (!data) {
        ...
        for (let off = 0; off < urls.length; off++) {
            ...
            const url = urls[idx];
            try {
                data = await callBatchGeneric(url, requests, timeout, label);
                lastGood.set(chainId, idx);
                clearCooldown(url);
                break;
            } catch (e) {
...
/**
 * Urutkan ulang RPC per chain berdasar skor historis dari rpcHealthCache.
 * Dipanggil sekali di awal sesi setelah cache di-hidrasi ke rpcStats.
 * Endpoint dengan skor lebih tinggi diletakkan di depan, sehingga sticky
 * lastGood (default 0) langsung memilih yang historisnya paling sehat.
 */
function reorderByHealth(scoreFn) {
    if (typeof scoreFn !== "function") return;
    let touched = 0;
    for (const [idStr, urls] of Object.entries(RPCS)) {
        const chainId = parseInt(idStr, 10);
        const label   = `EVM/${chainName(chainId)}`;
        const scored  = urls.map((u) => {
            const e = rpcStats.getEntry(label, u);
            return { url: u, score: e ? scoreFn(e) : 0.5 };
        });
        // Sort descending; stabil-ish (tie biarkan urutan asal lewat indeks asli).
        scored.sort((a, b) => b.score - a.score);
        const newUrls = scored.map((s) => s.url);
        if (newUrls.some((u, i) => u !== urls[i])) {
            RPCS[chainId] = newUrls;
            lastGood.set(chainId, 0);
            touched++;
        }
    }
    return touched;
...
/**
 * Snapshot status RPC per chain — dipakai dashboard live.
 * Returns Array<{ chainId, name, totalUrls, cooldownCount, cooldownUntil:number[] }>.
 */
function rpcChainStatus() {
    const now = Date.now();
    const out = [];
    for (const [idStr, urls] of Object.entries(RPCS)) {
        const chainId = parseInt(idStr, 10);
        ...
        for (const u of urls) {
            const until = cooldown.get(u);
            if (until && until > now) {
                cd++;
                if (until < nextFreeMs) nextFreeMs = until;
            }
        }
        out.push({
            chainId,
            ...
            cooldownCount:  cd,
            healthyCount:   urls.length - cd,
            nextFreeInMs:   isFinite(nextFreeMs) ? Math.max(0, nextFreeMs - now) : 0,
        });
    }
    return out;
}
...
module.exports = {
    ...
    rpcChainStatus, reorderByHealth,
};
